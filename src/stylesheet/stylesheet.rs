        ...
        let styles = raw_map
            ...
            .map(|(style_name, elements)| {
                ...
                for element in elements {
                    if let Some(c) = element.const_style {
                        final_style.const_style = Some(c);
                    }
                    if let Some(s) = element.default {
                        final_style.default = Some(s);
                    }
                    if let Some(s) = element.checked {
                        final_style.checked = Some(s);
                    }
                    if let Some(s) = element.focused {
                        final_style.focused = Some(s);
                    }
                    if let Some(s) = element.focus_key {
                        final_style.focus_key = Some(s);
                    }
                    if let Some(s) = element.edited {
                        final_style.edited = Some(s);
                    }
                    if let Some(s) = element.hovered {
                        final_style.hovered = Some(s);
                    }
                    if let Some(s) = element.pressed {
                        final_style.pressed = Some(s);
                    }
                    if let Some(s) = element.disabled {
                        final_style.disabled = Some(s);
                    }
                    if let Some(s) = element.user_1 {
                        final_style.user_1 = Some(s);
                    }
                    if let Some(s) = element.user_2 {
                        final_style.user_2 = Some(s);
                    }
                    if let Some(s) = element.user_3 {
                        final_style.user_3 = Some(s);
                    }
                    if let Some(s) = element.user_4 {
                        final_style.user_4 = Some(s);
                    }
                    if let Some(s) = element.any {
                        final_style.any = Some(s);
                    }
                }
                final_style
            })
